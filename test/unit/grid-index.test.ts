...
const worked: Location[] = [loc('#1', 1, 1, 1), loc('#2', 2, 2, 2), loc('#3', 5, 5, 1), loc('#4', 2, 3, 5)];

describe('GridIndex', () => {
  it('worked example: user (3,2) -> #2 and #4', () => {
    const idx = new GridIndex();
    idx.bulkLoad(worked);
    expect(idx.search({ x: 3, y: 2 }).map((h) => h.location.id).sort()).toEqual(['#2', '#4']);
  });
  ...
  it('handles large sparse coordinates (cell map stays small)', () => {
    ...
    expect(idx.search({ x: 1_000_000, y: 2_000_000 }).map((h) => h.location.id).sort()).toEqual(['far', 'near']);
  ...
  it('inserting a radius bigger than the current cell size triggers a rebuild and stays correct', () => {
    ...
    expect(idx.search({ x: 0, y: 0 }).map((h) => h.location.id).sort()).toEqual(['huge', 'small']);
