...
describe('loadLocationsFromFile', () => {
  it('loads a valid file', () => {
    const r = loadLocationsFromFile(fixture('locations.sample.json'), {
  ...
  it('aborts when more than the threshold fraction of records are invalid', () => {
    expect(() =>
      loadLocationsFromFile(fixture('locations.mostly-broken.json'), { abortInvalidFraction: 0.5 }),
    ).toThrow(/more than 50% .* invalid/i);
  });

  it('throws on a missing file', () => {
    expect(() =>
      loadLocationsFromFile(fixture('does-not-exist.json'), { abortInvalidFraction: 0.5 }),
    ).toThrow();
  });

  it('throws on invalid JSON', () => {
    expect(() =>
      loadLocationsFromFile(fixture('not-json.txt'), { abortInvalidFraction: 0.5 }),
    ).toThrow(/valid JSON/i);
  });

  it('throws on the wrong top-level shape', () => {
    expect(() =>
      loadLocationsFromFile(fixture('wrong-shape.json'), { abortInvalidFraction: 0.5 }),
    ).toThrow(/locations.*array/i);
  });
});
